...
void allocateBuffer(nixl_mem_t mem_type, int dev_id, size_t len, void* &addr)
{
    ...
    case VRAM_SEG: {
        ...
        assert(dev_id >= 0 && dev_id < devices.size());
        sycl::queue q(devices[dev_id]);
...
void releaseBuffer(nixl_mem_t mem_type, int dev_id, void* &addr)
{
    ...
    case DRAM_SEG:
        ...
        break;
    case VRAM_SEG: {
        // Use SYCL to free device memory
        auto devices = sycl::device::get_devices();
        assert(dev_id >= 0 && dev_id < devices.size());
        sycl::queue q(devices[dev_id]);
        sycl::free(addr, q);
...
void *getValidationPtr(nixl_mem_t mem_type, void *addr, size_t len)
{
    ...
    case VRAM_SEG: {
        ...
        auto devices = sycl::device::get_devices();
        assert(gpu_id >= 0 && gpu_id < devices.size());
        sycl::queue q(devices[gpu_id]);
        q.memcpy(ptr, addr, len).wait();
        return ptr;
    }
    default:
        std::cout << "Unsupported memory type!" << std::endl;
...
int main()
{
    ...
    int n_vram_dev = sycl::device::get_devices().size();

    std::cout << "Detected " << n_vram_dev << " xpu devices" << std::endl;
